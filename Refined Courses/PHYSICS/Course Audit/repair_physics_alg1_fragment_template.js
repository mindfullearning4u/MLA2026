...
function header(unit, lesson) {
  ...
  <div style="font-size: 0.95rem; font-weight: 700; letter-spacing: 0.02em;">
    &#128216; PHYSICS | Unit ${unit} | Lesson ${lesson}
  </div>
</div>`;
}
...
function removeOldHeader(html) {
  return html
    .replace(/^\s*<div[^>]*>\s*<strong>PHYSICS \| Unit \d{2} \| Lesson \d{2}<\/strong>\s*<\/div>\s*/i, '')
    .replace(/^\s*<h1[^>]*>PHYSICS Unit \d{2} Lesson \d{2}<\/h1>\s*/i, '');
}

function extractRoleBlock(html, page) {
  const role = pageInfo[page].role;
  const sectionPattern = new RegExp(`<section[^>]*>\\s*<h1[^>]*>\\s*${role.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*<\\/h1>[\\s\\S]*?<\\/section>`, 'i');
  const h1Pattern = new RegExp(`<h1[^>]*>\\s*${role.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*<\\/h1>`, 'i');
  const sectionMatch = html.match(sectionPattern);
  if (sectionMatch) {
    const titleMatch = sectionMatch[0].match(/<strong>Lesson Title:<\/strong>\s*([^<]+)/i);
    return { title: titleMatch ? titleMatch[1].trim() : '', body: html.replace(sectionMatch[0], '') };
  }
  return { title: '', body: html.replace(h1Pattern, '') };
}
...
function normalizeOpeningTag(attrs) {
  ...
  const style = cardStyle(rawClass);
  let nextAttrs = classMatch ? attrs.replace(/\bclass=(["']).*?\1/i, `class="${newClass}"`) : `${attrs} class="${newClass}"`;
  nextAttrs = /\bstyle=/i.test(nextAttrs) ? nextAttrs.replace(/\bstyle=(["']).*?\1/i, `style="${style}"`) : `${nextAttrs} style="${style}"`;
...
function convertSections(html) {
  return html
    ...
    .replace(/<div([^>]*)class=(["'])mla-tor-support-box\2([^>]*)>/gi, `<div class="mla-tor-support-box" style="${cardStyle('tor')}">`);
}
...
function normalizeLabels(html, page) {
  ...
  if (page === 'P06.html') {
    ...
    if (!/Part B/i.test(next)) next += `\n<div style="${cardStyle('check')}"><h2>Part B</h2><p>Use a diagram, model, table, or data detail to support your answer.</p></div>`;
    if (!/Part C/i.test(next)) next += `\n<div style="${cardStyle('check')}"><h2>Part C</h2><p>Write a final explanation that connects physics evidence, units, and reasoning.</p></div>`;
  }
  if (page === 'P07.html') {
  ...
  if ((page === 'P03.html' || page === 'P04.html') && !/Teachable Explanation/i.test(next)) {
    next += `\n<div style="${cardStyle('correct')}"><h2>Teachable Explanation</h2><p>A complete physics answer identifies the system, uses measured evidence or a model, and explains the physics relationship that supports the conclusion.</p></div>`;
  }
  return next;
}

function ensureTor(html) {
  const count = (html.match(/class=["']mla-tor-support-box["']/g) || []).length;
  if (count === 1) return html;
  let next = html.replace(/<div[^>]*class=["']mla-tor-support-box["'][^>]*>[\s\S]*?<\/div>/gi, '');
  next += `\n<div class="mla-tor-support-box" style="${cardStyle('tor')}">\n  <p style="font-size: 18px; font-weight: 700; margin: 0 0 6px 0;">Need Help?</p>\n  <p style="margin: 0 0 6px 0;">Contact your Teacher of Record if you cannot connect the model, graph, data, units, or reasoning step to the standard after reviewing the lesson steps.</p>\n  <p style="margin: 0;">Bring the page number, the evidence you used, and the exact reasoning step that is confusing.</p>\n</div>`;
  return next;
}

function normalizePage(html, pagePath, page) {
  const { unit, lesson } = ids(pagePath);
  const stripped = removeOldHeader(stripShell(html));
  const { title, body } = extractRoleBlock(stripped, page);
