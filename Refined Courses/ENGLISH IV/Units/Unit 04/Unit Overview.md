# ENGLISH IV - Unit 04 Overview: Research, Synthesis, and Extended Writing

---

## Unit Overview

This unit develops sustained research and synthesis skills. Students conduct research on topical issues, answer research questions, synthesize information from a variety of sources, analyze complex texts in depth, and create multimedia texts through digital formats.

Emphasis is placed on research design, synthesis, credibility, organization, tone, academic integrity, source discernment, extended writing, and multimedia communication. Students use research and source synthesis to strengthen analytical and argumentative writing.

Instruction and assessment are aligned to Florida B.E.S.T. ELA Grade 12 Standards with an emphasis on evaluation, synthesis, argumentation, and communication. Students are expected to demonstrate advanced writing fluency and conceptual understanding through research, complex text analysis, source evaluation, synthesis, digital publication, and extended writing.

---

## Primary B.E.S.T. Benchmarks

| Standard Code | Standard Description |
|--------------|----------------------|
| ELA.12.C.1.4 | Write an in-depth analysis of complex texts using logical organization and appropriate tone and voice, demonstrating a thorough understanding of the subject. |
| ELA.12.C.4.1 | Conduct research on a topical issue to answer a question and synthesize information from a variety of sources. |
| ELA.12.C.5.2 | Create, publish, and share multimedia texts through a variety of digital formats. |

---

## Prerequisite and Supporting Concepts

| Standard Code | Standard Description |
|--------------|----------------------|
| ELA.12.R.2.4 | Compare the development of multiple arguments in related texts, evaluating the validity of the claims, the authors’ reasoning, use of the same information, and/or the authors’ rhetoric. |
| ELA.12.C.1.3 | Write arguments to support claims based on an in-depth analysis of topics or texts, using valid reasoning and credible evidence from sources, elaboration, and demonstrating a thorough understanding of the subject. |
| ELA.12.C.3.1 | Follow the rules of standard English grammar, punctuation, capitalization, and spelling appropriate to grade level. |
| ELA.12.V.1.1 | Integrate academic vocabulary appropriate to grade level in speaking and writing. |

---

## Lesson Breakdown

| Lesson | Lesson Title | Primary Standard(s) | Assessment |
|--------|--------------|---------------------|------------|
| Lesson 1 | Topical Issues and Research Questions | ELA.12.C.4.1 | Quiz |
| Lesson 2 | Source Discernment and Credibility | ELA.12.C.4.1 | Quiz |
| Lesson 3 | Synthesizing Information from a Variety of Sources | ELA.12.C.4.1 | Quiz |
| Lesson 4 | In-Depth Analysis of Complex Texts | ELA.12.C.1.4 | Quiz |
| Lesson 5 | Organizing Extended Analytical Writing | ELA.12.C.1.4 | Quiz |
| Lesson 6 | Tone, Voice, Citation, and Academic Integrity | ELA.12.C.1.4 | Quiz |
| Lesson 7 | Creating and Sharing Multimedia Texts | ELA.12.C.5.2 | Quiz |
| Lesson 8 | Unit Assessment | All Unit Benchmarks | Unit Assessment |

---

## Unit Assessment Alignment

The Unit Assessment evaluates student mastery of all benchmarks addressed in this unit. Students must demonstrate consistent proficiency across:

- conceptual understanding;
- research on topical issues;
- synthesis of information from a variety of sources;
- in-depth complex text analysis;
- logical organization, tone, and voice;
- citation and academic integrity;
- multimedia texts through digital formats.

Assessment items require students to conduct research on a topical issue, answer a research question, synthesize information from a variety of sources, write an in-depth analysis of complex texts with logical organization and appropriate tone and voice, and create, publish, and share multimedia texts through a variety of digital formats.

---

## Mastery Expectation

Students are expected to demonstrate a minimum of 80% mastery across all required instructional components, including Notebook Evidence, Checkpoints, Quizzes, and the Unit Assessment.

Students are not advanced to the next unit until mastery is achieved. Mastery is determined through consistent performance across multiple measures and is not based on a single assessment.

The Unit Assessment is required for all students and contributes to the overall mastery determination.

Students participating in the Accelerated Path must meet additional expectations. Please refer to the official Accelerated Path Overview document for full eligibility, requirements, and continuation criteria.

---

## Problem-Solving Connection

This unit connects inquiry, reading, writing, and technology. Students learn to investigate topical issues, synthesize varied sources, analyze complex texts, and communicate findings clearly and responsibly through extended writing and multimedia formats.

---
