...
const unit = path.join(root, "PHYSICS", "Units", "Unit 06");
...
for (const lesson of lessons) {
  ...
  for (const page of ["P01", "P02", "P03", "P04", "P05", "P06", "P07"]) {
    ...
    if (!/diagram|table|chart|model|scale|evidence|frame|reliability|decision|visual/i.test(html)) failures.push(`${file} missing explicit visual/model/evidence language`);
  }
...
for (const file of xmlFiles) {
  ...
  if (!/table|diagram|chart|model|scale|evidence|frame|reliability|decision|visual/i.test(xml)) failures.push(`Missing embedded visual/data/model stimulus: ${file}`);
}

const expectedCounts = new Map([
  ["PHYS_U06_Pretest_MoodleXML.xml", 10],
  ["PHYS_U06_UnitAssessment_MoodleXML.xml", 40]
]);
for (let n = 1; n <= 8; n++) {
  const lesson = String(n).padStart(2, "0");
  expectedCounts.set(`PHYS_U06_L${lesson}_GuidedPractice_MoodleXML.xml`, 5);
  if (n <= 7) expectedCounts.set(`PHYS_U06_L${lesson}_Quiz_MoodleXML.xml`, 25);
}
