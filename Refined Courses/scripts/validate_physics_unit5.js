...
const unit = path.join(root, "PHYSICS", "Units", "Unit 05");
...
for (const lesson of lessons) {
  ...
  for (const page of ["P01", "P02", "P03", "P04", "P05", "P06", "P07"]) {
    ...
    if (!/diagram|table|chart|model|field|circuit|visual|device/i.test(html)) failures.push(`${file} missing explicit visual/model language`);
  }
...
for (const file of xmlFiles) {
  ...
  if (!/table|diagram|chart|model|field|circuit|visual|device/i.test(xml)) failures.push(`Missing embedded visual/data/model stimulus: ${file}`);
}

const expectedCounts = new Map([
  ["PHYS_U05_Pretest_MoodleXML.xml", 10],
  ["PHYS_U05_UnitAssessment_MoodleXML.xml", 40]
]);
for (let n = 1; n <= 8; n++) {
  const lesson = String(n).padStart(2, "0");
  expectedCounts.set(`PHYS_U05_L${lesson}_GuidedPractice_MoodleXML.xml`, 5);
  if (n <= 7) expectedCounts.set(`PHYS_U05_L${lesson}_Quiz_MoodleXML.xml`, 25);
}
