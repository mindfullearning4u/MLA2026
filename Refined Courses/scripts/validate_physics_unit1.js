...
const unit = path.join(root, "PHYSICS", "Units", "Unit 01");
...
for (const lesson of lessons) {
  ...
  for (const page of ["P01", "P02", "P03", "P04", "P05", "P06", "P07"]) {
    ...
    if (/teacher will|teacher check|wait for teacher guidance|teacher will show|teacher will explain/i.test(html)) {
      failures.push(`Prohibited teacher-led language: ${file}`);
    }
    if (page === "P04" && (html.match(/Worked Example [123]:/g) || []).length !== 3) {
      failures.push(`${file} does not contain exactly three worked examples`);
    }
  }
...
const expectedCounts = new Map([
  ["PHYS_U01_Pretest_MoodleXML.xml", 10],
  ["PHYS_U01_UnitAssessment_MoodleXML.xml", 40]
]);
for (let n = 1; n <= 8; n++) {
  const lesson = String(n).padStart(2, "0");
  expectedCounts.set(`PHYS_U01_L${lesson}_GuidedPractice_MoodleXML.xml`, 5);
  if (n <= 7) expectedCounts.set(`PHYS_U01_L${lesson}_Quiz_MoodleXML.xml`, 25);
}
