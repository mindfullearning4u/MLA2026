...
function deepTeaching(l) {
  const common = `<p>Physics often feels difficult because one sentence can contain several ideas at once. Slow the sentence down. First identify the object or system. Next identify the quantity being measured. Then identify the unit, direction, graph axis, table column, or model feature. Only after that should you calculate or choose an answer.</p>`;
  const blocks = {
    "01": `${common}${table(["Student Question", "What To Do"], [["What is the claim?", "Find the sentence that answers the investigation question."], ["What is the evidence?", "Find the measured value, observation, table row, or model feature."], ["What is the reasoning?", "Explain the physics idea that connects the evidence to the claim."]])}<p><strong>Teacher move:</strong> A veteran teacher would not let students say "because it happened." The student must point to the exact data or observation and explain how it supports the claim.</p>`,
    "02": `${common}${table(["Measurement Step", "Why It Matters", "Example"], [["Choose the quantity", "The quantity controls the tool.", "distance"], ["Choose the tool", "The tool must measure that quantity.", "meter stick"], ["Record the number", "The value alone is not enough.", "0.75"], ["Record the unit", "The unit tells what the value means.", "0.75 m"]])}<p><strong>Teacher move:</strong> If a student writes only a number, stop and ask, "A number of what?" Physics answers are incomplete without units.</p>`,
    "03": `${common}${table(["Graph Decision", "How To Decide"], [["Horizontal axis", "Use the variable that is controlled, ordered, or used to compare the measurement. In motion, this is usually time."], ["Vertical axis", "Use the measured result, such as position or velocity."], ["Units", "Put units in the axis labels so the graph can be interpreted without guessing."], ["Pattern", "Look for equal changes, increasing changes, decreasing changes, or no change."]])}<p><strong>Teacher move:</strong> Before graphing, a veteran teacher asks students to read the table out loud: "At 0 seconds the position is 0 meters; at 1 second the position is 2 meters." That prevents students from graphing numbers without meaning.</p>`,
    "04": `${common}${table(["Idea", "Student-Friendly Meaning", "How To Check"], [["Precision", "Repeated measurements are close to each other.", "Compare the trial values."], ["Accuracy", "A measurement is close to the accepted value.", "Compare to a known or accepted value."], ["Outlier", "One value is far from the pattern.", "Ask whether it may be an error or a real event."], ["Uncertainty", "Measurements have limits.", "Use the tool precision and repeated trials."]])}<p><strong>Teacher move:</strong> Do not let students erase data just because it looks strange. They must explain why the value should be checked.</p>`,
    "05": `${common}${table(["Quantity Type", "Must Include", "Example", "Not Enough"], [["Scalar", "magnitude and unit", "12 m/s", "east by itself"], ["Vector", "magnitude, unit, and direction", "12 m/s east", "12 m/s without direction"], ["Distance", "how much path length", "50 m", "50 m north if direction matters"], ["Displacement", "change in position with direction", "50 m north", "50 m without direction"]])}<p><strong>Teacher move:</strong> A veteran teacher draws arrows for vectors. The arrow length represents magnitude, and the arrow points in the direction of the quantity.</p>`,
    "06": `${common}${table(["Formula", "Meaning", "When To Use It"], [["speed = distance / time", "path length divided by time", "when direction is not required"], ["velocity = displacement / time", "change in position with direction divided by time", "when direction matters"], ["displacement = final position - starting position", "how far and which direction from start to finish", "before calculating velocity"]])}<p><strong>Teacher move:</strong> Students often divide numbers before deciding whether they need speed or velocity. First ask: "Does the answer need direction?" If yes, use displacement and keep the direction.</p>`,
    "07": `${common}${table(["Formula", "Meaning", "Student Check"], [["acceleration = change in velocity / time", "how quickly velocity changes", "compare velocity values first"], ["change in velocity = final velocity - starting velocity", "how much velocity changed", "subtract in the correct order"], ["constant acceleration", "same velocity change in each equal time interval", "look for the same change each row"]])}<p><strong>Teacher move:</strong> A student may think acceleration means fast. Correct that immediately: acceleration means velocity is changing. A slow object can accelerate, and a fast object can have zero acceleration if its velocity stays constant.</p>`,
    "08": `${common}${table(["Unit 1 Skill", "What Student Must Show"], [["Inquiry", "Use claim, evidence, and reasoning."], ["Safety", "Reject unsafe or unapproved investigations."], ["Measurement", "Use correct tools and units."], ["Data/graphs", "Read labels, axes, and patterns."], ["Vectors/motion", "Keep direction when direction is part of the quantity."], ["Acceleration", "Use velocity change, not final velocity alone."]])}<p><strong>Teacher move:</strong> In a synthesis lesson, the student must decide which Unit 1 tool fits the problem before solving. The goal is not memorizing separate facts; it is choosing the correct evidence routine.</p>`
  };
  return blocks[l.no] || common;
}

function misconceptionLadder(l) {
  return `<p><strong>Remedial support:</strong> Read one row, one label, or one sentence at a time. Say what the number or model feature means in words before calculating.</p>
<p><strong>Standard support:</strong> Use the lesson routine independently: identify the quantity, copy the evidence, choose the vocabulary or formula, and explain the answer.</p>
<p><strong>Accelerated extension:</strong> Explain how the answer would change if one part of the evidence changed, such as a different direction, time, trial value, or graph pattern.</p>
<p><strong>Question to ask yourself:</strong> What could a student misunderstand here, and what evidence in the lesson removes that confusion?</p>`;
}
...
${section("#334155", "#f8fafc", "Before the Worked Examples", `<p>Do not start by hunting for an answer choice. Start by setting up the physics. The setup is part of the answer.</p>${deepTeaching(l)}`)}
