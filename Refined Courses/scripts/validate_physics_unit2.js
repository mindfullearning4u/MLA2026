...
const unit = path.join(root, "PHYSICS", "Units", "Unit 02");
...
const expectedCounts = new Map([
  ["PHYS_U02_Pretest_MoodleXML.xml", 10],
  ["PHYS_U02_UnitAssessment_MoodleXML.xml", 40]
]);
for (let n = 1; n <= 8; n++) {
  const lesson = String(n).padStart(2, "0");
  expectedCounts.set(`PHYS_U02_L${lesson}_GuidedPractice_MoodleXML.xml`, 5);
  if (n <= 7) expectedCounts.set(`PHYS_U02_L${lesson}_Quiz_MoodleXML.xml`, 25);
}
