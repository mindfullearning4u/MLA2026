...
function header(unit, lesson) {
  ...
  <div style="font-size: 0.95rem; font-weight: 700; letter-spacing: 0.02em;">
    &#128216; ENVIRONMENTAL SCIENCE | Unit ${unit} | Lesson ${lesson}
  </div>
</div>`;
}
...
function normalizeLabels(html, page) {
  ...
  if (page === 'P06.html') {
    ...
    if (!/Part B/i.test(next)) next += `\n<div style="${cardStyle('check')}"><h2>Part B</h2><p>Use a map, model, graph, table, source, or case detail to support your answer.</p></div>`;
    if (!/Part C/i.test(next)) next += `\n<div style="${cardStyle('check')}"><h2>Part C</h2><p>Write a final explanation that connects environmental evidence, system relationships, and reasoning.</p></div>`;
  }
  if (page === 'P07.html') {
    ...
    if (!/Submission Workflow/i.test(next)) next += `\n<div style="${cardStyle('check')}"><h2>Submission Workflow</h2><p>Submit the checkpoint response, notebook evidence, and any required correction work in Moodle. If returned, revise the evidence and reasoning before resubmission.</p></div>`;
  ...
  if (page === 'P04.html' && !/Step 1/i.test(next)) {
    next = next.replace(/(<h2>Worked Example 1[\s\S]*?<\/h2>)/i, '$1\n<p><strong>Step 1:</strong> Identify the environmental system, source, data display, map, model, or case evidence before choosing or writing an answer.</p>');
  }
  if (page === 'P04.html' && !/Incorrect:/i.test(next)) {
    next += `\n<div style="${cardStyle('mistake')}"><h2>Incorrect Reasoning Check</h2><p><strong>Incorrect:</strong> Choosing an environmental answer because it sounds familiar without matching it to the specific evidence, system, source, or tradeoff.</p></div>`;
  }
  if ((page === 'P03.html' || page === 'P04.html') && !/Teachable Explanation/i.test(next)) {
    next += `\n<div style="${cardStyle('correct')}"><h2>Teachable Explanation</h2><p>A complete environmental science answer names the evidence, identifies the system relationship or tradeoff, and explains how that evidence supports the claim.</p></div>`;
  }
  return next;
}
...
function ensureTor(html) {
  ...
  const boxes = [...next.matchAll(/<(div|aside)[^>]*class=(["'])mla-tor-support-box\2[^>]*>[\s\S]*?<\/\1>/gi)];
  if (boxes.length > 1) {
