...
function header(unit, lesson) {
  ...
  <div style="font-size: 0.95rem; font-weight: 700; letter-spacing: 0.02em;">
    &#128216; CHEMISTRY | Unit ${unit} | Lesson ${lesson}
  </div>
</div>`;
}
...
function extractTitle(html) {
  ...
  const title = raw
    .replace(/^CHEMISTRY Unit \d+ Lesson \d+\s*/i, '')
...
function normalizeOpeningTag(tagName, attrs) {
  ...
  const style = /tor|teacher/i.test(rawClass) || !styleMatch ? cardStyle(rawClass) : styleMatch[2];
  const classAttr = classMatch ? attrs.replace(/\bclass=(["']).*?\1/i, `class="${newClass}"`) : `${attrs} class="${newClass}"`;
  const styleAttr = styleMatch ? classAttr.replace(/\bstyle=(["']).*?\1/i, `style="${style}"`) : `${classAttr} style="${style}"`;
  return `<div${styleAttr}>`;
}

function convertBlocks(html) {
  return html
    .replace(/<section([^>]*)>/gi, (_, attrs) => normalizeOpeningTag('section', attrs))
    .replace(/<\/section>/gi, '</div>')
    .replace(/<div([^>]*)>/gi, (_, attrs) => normalizeOpeningTag('div', attrs));
}
...
function normalizeLabels(html, page) {
  ...
  if (page === 'P06.html') {
    ...
    if (!/Part C/i.test(next)) next += `\n<div style="${cardStyle('check')}"><h2>Part C</h2><p>Write a final explanation that connects chemistry evidence, vocabulary, and reasoning.</p></div>`;
  }
  ...
  if (page === 'P04.html' && !/Step 1/i.test(next)) {
    next = next.replace(/(<h2>Worked Example 1[\s\S]*?<\/h2>)/i, '$1\n<p><strong>Step 1:</strong> Identify the claim, model, measurement, data table, or chemical evidence before choosing or writing an answer.</p>');
  }

  if ((page === 'P03.html' || page === 'P04.html') && !/Teachable Explanation/i.test(next)) {
    next += `\n<div style="${cardStyle('correct')}"><h2>Teachable Explanation</h2><p>A complete chemistry answer names the claim, uses measured or modeled evidence, and explains why that evidence supports the conclusion.</p></div>`;
  }
...
function ensureTor(html) {
  let next = html
    .replace(/class=(["'])tor\1/gi, 'class="mla-tor-support-box"')
    .replace(/class=(["'])teacher[^"']*\1/gi, 'class="mla-tor-support-box"');
  const matches = [...next.matchAll(/<div[^>]*class=(["'])mla-tor-support-box\1[^>]*>[\s\S]*?<\/div>/gi)];
  if (matches.length > 1) {
    const keep = matches[matches.length - 1][0];
    for (let i = 0; i < matches.length - 1; i += 1) {
      next = next.replace(matches[i][0], '');
    }
    if (!/style=/.test(keep)) next = next.replace(keep, keep.replace('<div', `<div style="${cardStyle('tor')}"`));
  }
  if (matches.length === 0) {
    next += `\n<div class="mla-tor-support-box" style="${cardStyle('tor')}">\n  <p style="font-size: 18px; font-weight: 700; margin: 0 0 6px 0;">Need Help?</p>\n  <p style="margin: 0 0 6px 0;">Contact your Teacher of Record if you cannot connect the model, data, chemical evidence, or reasoning step to the standard after reviewing the lesson steps.</p>\n  <p style="margin: 0;">Bring the page number, the evidence you used, and the exact reasoning step that is confusing.</p>\n</div>`;
  }
  return next;
}
