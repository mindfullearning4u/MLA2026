...
if (prodFiles.length < 9) fail(`Expected at least 9 course production files; found ${prodFiles.length}.`);
...
const weak = /important because it is important|answers may vary|placeholder|TBD|candidate resource|for approval/i;
...
if (weakHits.length) fail(`Weak/placeholder wording found: ${weakHits.map(rel).slice(0, 8).join(", ")}`);
...
for (const name of requiredProd) {
  ...
  if (!fs.existsSync(file)) fail(`Missing production file ${name}.`);
}

const alignmentFile = path.join(productionRoot, "PHASE_2A_C_BEST_COMMON_CORE_SAT_ACT_ALIGNMENT.md");
if (fs.existsSync(alignmentFile)) {
  const text = read(alignmentFile);
  for (const term of ["CPALMS", "FDOE", "Florida", "Common Core", "SAT", "ACT"]) {
    if (!text.includes(term)) fail(`Alignment file missing ${term}.`);
  }
}
...
for (let unit = 1; unit <= 6; unit++) {
  ...
  const audit = path.join(auditRoot, `SOC_U${String(unit).padStart(2, "0")}_UNIT_COMPLETION_AUDIT.md`);
...
const stereotypeBoundaryCheck = [...htmlFiles, ...xmlFiles].filter(file => !/stereotyp|unsupported claim|respectful/i.test(read(file)));
if (stereotypeBoundaryCheck.length) fail(`Sociology stereotype/evidence boundary missing: ${stereotypeBoundaryCheck.map(rel).slice(0, 8).join(", ")}`);
...
fs.writeFileSync(path.join(auditRoot, "SOC_FINAL_COURSE_COMPLETION_AUDIT.md"), `# Sociology Final Course Completion Audit
...
- Embedded sociology visual and source supports are present in lessons and XML questions.
- Moodle XML is the only production assessment format; no legacy production assessment files are present.
- No social science simulation requirement or approval placeholder appears in lessons or assessment files.
- Sociology safety boundary is present: students explain evidence and social patterns without stereotyping, labeling, or guessing.
- Florida/CPALMS/FDOE, Common Core, SAT, and ACT support alignment files are present.
