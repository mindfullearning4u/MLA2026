...
for (let unit = 1; unit <= 6; unit++) {
  ...
  const audit = path.join(auditRoot, `PSY_U${String(unit).padStart(2, "0")}_UNIT_COMPLETION_AUDIT.md`);
...
const noDiagnosisCheck = [...htmlFiles, ...xmlFiles].filter(file => !/diagnos|unsupported diagnosis|diagnosing/i.test(read(file)));
if (noDiagnosisCheck.length) fail(`Psychology safety/diagnosis boundary missing: ${noDiagnosisCheck.map(rel).slice(0, 8).join(", ")}`);
...
fs.writeFileSync(path.join(auditRoot, "PSY_FINAL_COURSE_COMPLETION_AUDIT.md"), `# Psychology Final Course Completion Audit
...
- Embedded psychology visual and source supports are present in lessons and XML questions.
...
- Psychology safety boundary is present: students explain evidence and concepts without diagnosing, labeling, or guessing.
