...
const unitDirs = fs.existsSync(unitsRoot) ? fs.readdirSync(unitsRoot, { withFileTypes: true }).filter(d => d.isDirectory()) : [];
if (unitDirs.length !== 6) fail(`Expected 6 unit folders, found ${unitDirs.length}`);
for (let u = 1; u <= 6; u++) {
  const unitDir = path.join(unitsRoot, `Unit ${pad(u)}`);
  ...
  const pretest = path.join(unitDir, `HPE_U${pad(u)}_Pretest.xml`);
  if (!fs.existsSync(pretest)) fail(`Missing Unit ${u} pretest`);
  for (let l = 1; l <= 8; l++) {
    const lessonDir = path.join(unitDir, `Lesson ${pad(l)}`);
    for (let p = 1; p <= 7; p++) {
    ...
    if (!fs.existsSync(path.join(lessonDir, `HPE_U${pad(u)}_L${pad(l)}_GuidedPractice.xml`))) fail(`Missing guided practice Unit ${u} Lesson ${l}`);
    if (l < 8 && !fs.existsSync(path.join(lessonDir, `HPE_U${pad(u)}_L${pad(l)}_Quiz.xml`))) fail(`Missing quiz Unit ${u} Lesson ${l}`);
    if (l === 8 && !fs.existsSync(path.join(lessonDir, `HPE_U${pad(u)}_UnitAssessment.xml`))) fail(`Missing unit assessment Unit ${u}`);
  }
}
...
for (const f of xml) {
  ...
  for (const [idx, chunk] of chunks.entries()) {
    ...
    if (/Visual stimulus/.test(chunk) && !/<table>/.test(chunk)) fail(`Question ${idx + 1} in ${rel(f)} has visual label without table stimulus`);
  }
}
...
for (const f of html) {
  const text = read(f);
  if (/P02|P04|P06/.test(path.basename(f)) && !/(mla-visual|<table|fitness planning chart|nutrition-label model|wellness strategy map|safety decision pathway|wellness portfolio organizer)/i.test(text)) htmlMissingVisual++;
  if (/teacher will explain|teacher will show|wait for teacher guidance|teacher check|live teacher|not rely on live teacher/i.test(text)) prohibitedTeacher++;
...
for (let u = 1; u <= 6; u++) {
  const audit = path.join(root, "Course Audit", `HPE_U${pad(u)}_UNIT_COMPLETION_AUDIT.md`);
  if (!fs.existsSync(audit)) fail(`Missing unit audit ${u}`);
}

const report = `# Health & Physical Education Final Course Completion Audit
...
write(path.join(root, "Course Audit", "HPE_FINAL_COURSE_COMPLETION_AUDIT.md"), report);
